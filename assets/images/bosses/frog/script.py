...
def process_spritesheet(input_path, output_path):
    ...
    # Nova spritesheet: todos os frames com largura do maior
    ...
    for i, (start, end) in enumerate(frames):
        ...
        # Centraliza o frame na largura padrão
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python script.py <imagem.png> [saida.png]")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else "spritesheet_trimmed.png"
    
    process_spritesheet(input_file, output_file)